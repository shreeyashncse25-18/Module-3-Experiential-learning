...
function applyFilters() {
  ...
  books.forEach(function(b) {
    if (activeType !== 'all' && b.type !== activeType) return;
    if (q && !b.title.toLowerCase().includes(q) && !b.author.toLowerCase().includes(q) && !b.isbn.includes(q)) return;
    shown++;
    const tr = document.createElement('tr');
    tr.innerHTML = '<td>' + b.id + '</td><td><div class="book-title">' + b.title + '</div><div class="book-isbn">' + b.isbn + '</div></td><td>' + b.author + '</td><td><span class="' + bc[b.type] + '">' + b.type + '</span></td>';
...
function buildAuthors() {
  const map = {};
  books.forEach(function(b) {
    if (!map[b.author]) map[b.author] = {count: 0, types: []};
    map[b.author].count++;
    if (!map[b.author].types.includes(b.type)) map[b.author].types.push(b.type);
  });
  ...
  Object.keys(map).forEach(function(name) {
    ...
    let badges = '';
    map[name].types.forEach(function(tp) { badges += '<span class="badge ' + bc[tp] + '">' + tp + '</span> '; });
    card.innerHTML = '<h4>' + name + '</h4><p class="count">' + map[name].count + ' book(s)</p><div>' + badges + '</div>';
